...
//                                        openUrl(context, uri, allowCustom, false);
        ...
        try {
            String groupurl=uri.toString().substring(uri.toString().lastIndexOf("/")+1);
            Intent intent = new Intent(Intent.ACTION_VIEW, Uri.parse("http://t.me/"+groupurl));
            if (internalUri) {
    ...
    public static boolean isInternalUri(Uri uri, boolean forceBrowser[]) {
        ...
        } else if ("telegram.me".equals(host) || "t.me".equals(host) || "telesco.pe".equals(host)|| "biying.im".equals(host)) {
            String path = uri.getPath();
            if (path != null && path.length() > 1) {
                path = path.substring(1).toLowerCase();
                if (path.equals("iv")) {
